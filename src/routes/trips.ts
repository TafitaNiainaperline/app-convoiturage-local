...
router.get('/', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const filtre: Record<string, any> = { statut: 'disponible' };

    if (depart) filtre['depart.ville'] = new RegExp(depart as string, 'i');
    if (arrivee) filtre['arrivee.ville'] = new RegExp(arrivee as string, 'i');
    if (date) {
      const debut = new Date(date as string);
      const fin = new Date(date as string);
      fin.setDate(fin.getDate() + 1);
      filtre.dateDepart = { $gte: debut, $lt: fin };
    }
    if (places) filtre.placesDisponibles = { $gte: parseInt(places as string) };
...
router.get('/conducteur/mes-trajets', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.put('/:id', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!trajet) {
      res.status(404).json({ message: 'Trajet introuvable ou non autorisé.' });
      return;
    }
